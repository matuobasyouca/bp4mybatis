        ...
        if (Constant.refreshTime() > refreshTime) {
            ...
            for (SystemCode c :
                ...
                if ("PermissionRule".equals(c.getCodeType())) {
                    for(String role : c.getCodeName().split(",")) {
                        if(!ValidUtil.isEmpty(role)) {
                            for(String entity : c.getCodeValue().split(",")) {
                                if(!ValidUtil.isEmpty(entity)) {
                                    PermissionRule rule = new PermissionRule();
                                    rule.setExps(c.getCodeDesc());
                                    rule.setRoles("," + role + ",");
                                    rule.setFromEntity("," + entity + ",");
